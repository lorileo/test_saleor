...
def get_braintree_gateway(sandbox_mode, merchant_id, public_key, private_key):
    ...
    if not all([merchant_id, private_key, public_key]):
        raise ImproperlyConfigured("Incorrectly configured Alipay gateway.")
    ...
    alipay_client_config.app_id = merchant_id
...
def get_client_token(
    ...
    client = get_braintree_gateway(**config.connection_params)
    # if not token_config:
    #     return client.page_execute(http_method="GET")
    request = create_token_params(config, token_config)
...
def create_token_params(config: GatewayConfig, token_config: TokenConfig) -> dict:
    ...
    model.out_trade_no = "202005020000226"
